...
from utils_training import (get_number_of_genes, load_and_split_data, perform_pca_and_plot, 
                            ...
                            integrate_ADTs_to_RNA,integrate_ADTs_to_RNA_2, reinitialize_training_with_new_discriminator,
                            train_new_discriminator, plot_integrated_latent_space_all, plot_integrated_latent_space_all_onlyADT,
                            cache_model_outputs_ADT, plot_integrated_latent_space_full_data_all, plot_integrated_latent_space_full_data_all_onlyADT,)
...
epochs_RNA_intermediary_integration = 200
...
def main():
    ...
    train_new_discriminator(new_trainer, source_technology, target_technology, train_datasets, OUTDIR, num_epochs=20) 
    ...
    for i in range(iterations):
        ...
        #evaluate latent space
        div_score = evaluate_latent_space(test, new_trainer, LABEL, selected_techs = ADT_TECHS)
        print("divergence score: ", div_score)
        ...
        new_vae_lut = new_trainer.get_trained_vae_lut()
        trainer.set_vae_lut(new_vae_lut)
        target_technology_2 = pre_integration_source
        source_technology_2 = 'AB'
        print("integrating ", source_technology_2, " to ", target_technology_2)
        iteration, cachedir = integrate_ADTs_to_RNA_2(epochs_RNA_intermediary_integration, trainer, full, train_datasets, test, target_technology_2, source_technology_2, iteration, OUTDIR, LABEL)

        target_technology_2 = pre_integration_source
        source_technology_2 = 'lectin'
        print("integrating ", source_technology_2, " to ", target_technology_2)
        iteration, cachedir = integrate_ADTs_to_RNA_2(epochs_RNA_intermediary_integration, trainer, full, train_datasets, test, target_technology_2, source_technology_2, iteration, OUTDIR, LABEL)
        #evaluate latent space
        div_score = evaluate_latent_space(test, trainer, LABEL, selected_techs = ADT_TECHS)
        print("divergence score: ", div_score)
        out = OUTDIR.joinpath(f'divergence_score_intermediary_RNA_{iteration}_{source_technology_2}_ADT_TECH.csv')
        with open(out, 'w', newline='') as file:
            writer = csv.writer(file)
            writer.writerow([div_score])

        plot_integrated_latent_space_all(trainer, test, iteration, target_technology_2, source_technology_2, TECHS, LABEL, OUTDIR, LABEL2=LABEL_2, cachedir=cachedir, inter_trainig  = True)
        inter = cache_model_outputs(trainer, full, target_technology_2, source_technology_2, OUTDIR, LABEL, iteration, 'AB', cachedir=cachedir)
        match_cells(OUTDIR, target_technology_2, source_technology_2, inter, iteration = i, inter_trainig = True)
        
        cachedir = OUTDIR / f'2.75_intermediary_RNA_{iteration}_{source_technology}_{target_technology}_full' / 'model'
        if not cachedir.exists():
            cachedir.mkdir(parents=True)
        plot_integrated_latent_space_full_data_all(trainer, full, iteration, target_technology_2, source_technology_2, TECHS, LABEL, OUTDIR, LABEL2=LABEL_2, cachedir=cachedir, inter_trainig  = True)

        cachedir = OUTDIR / f'2.75_intermediary_RNA_{iteration}_ONLY_ADT' / 'model'
        if not cachedir.exists():
            cachedir.mkdir(parents=True)
        plot_integrated_latent_space_all_onlyADT(trainer, test, iteration, target_technology_2, source_technology_2, TECHS, LABEL, OUTDIR, LABEL2=LABEL_2, cachedir=cachedir, inter_trainig  = True)
        inter = cache_model_outputs_ADT(trainer, full, target_technology_2, source_technology_2, OUTDIR, LABEL, iteration, 'AB', cachedir=cachedir)
        match_cells(OUTDIR, 'AB', 'lectin', inter, iteration = i, inter_trainig = True)
        
        cachedir = OUTDIR / f'2.75_intermediary_RNA_{iteration}_ONLY_ADT_full' / 'model'
        if not cachedir.exists():
            cachedir.mkdir(parents=True)
        plot_integrated_latent_space_full_data_all_onlyADT(trainer, full, iteration, target_technology_2, source_technology_2, TECHS, LABEL, OUTDIR, LABEL2=LABEL_2, cachedir=cachedir, inter_trainig  = True)


        vae_lut = trainer.get_trained_vae_lut()
        new_trainer.set_vae_lut(vae_lut)
