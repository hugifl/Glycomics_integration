...
from utils_training import (get_number_of_genes, load_and_split_data, perform_pca_and_plot, 
                            ...
                            integrate_ADTs_to_RNA,integrate_ADTs_to_RNA_2, reinitialize_training_with_new_discriminator, train_new_discriminator)

# Enable eager execution for TensorFlow
tf.compat.v1.enable_eager_execution()

# Define constants and paths
OUTDIR = Path('/cluster/scratch/hugifl/9_glycomics_8_RNA_regularized_inter_2')
TECHS = ['RNA', 'AB','lectin']
ADT_TECHS = ['AB','lectin']
...
def main():
    ...
    for i in range(iterations):
        ...
        plot_integrated_latent_space(trainer, test, iteration, target_technology_2, source_technology_2, TECHS, LABEL, OUTDIR, LABEL2=LABEL_2, cachedir=cachedir, inter_trainig  = True)
        cachedir = OUTDIR / f'2.75_intermediary_RNA_{iteration}_{source_technology_2}_lectin' / 'model'
        if not cachedir.exists():
            cachedir.mkdir(parents=True)
        vae_lut = trainer.get_trained_vae_lut()
        new_trainer.set_vae_lut(vae_lut)
        plot_integrated_latent_space(new_trainer, test, iteration, 'AB', 'lectin', TECHS, LABEL, OUTDIR, LABEL2=LABEL_2, cachedir=cachedir, inter_trainig  = True)
        inter = cache_model_outputs(new_trainer, full, 'AB', 'lectin', OUTDIR, LABEL, iteration, 'AB', cachedir=cachedir)
        ...
        plot_integrated_latent_space(trainer, test, iteration, target_technology_2, source_technology_2, TECHS, LABEL, OUTDIR, LABEL2=LABEL_2, cachedir=cachedir, inter_trainig  = True)
        cachedir = OUTDIR / f'2.75_intermediary_RNA_{iteration}_{source_technology_2}_AB' / 'model'
        if not cachedir.exists():
            cachedir.mkdir(parents=True)
        vae_lut = trainer.get_trained_vae_lut()
        new_trainer.set_vae_lut(vae_lut)
        plot_integrated_latent_space(new_trainer, test, iteration, 'lectin', 'AB', TECHS, LABEL, OUTDIR, LABEL2=LABEL_2, cachedir=cachedir, inter_trainig  = True)
        inter = cache_model_outputs(new_trainer, full, 'lectin', 'AB', OUTDIR, LABEL, iteration, 'AB', cachedir=cachedir)
        match_cells(OUTDIR, 'lectin', 'AB', inter, iteration = i, inter_trainig = True)
